...
export function getSchedulingOverview(view = "all", entityId = "") {
  const query = new URLSearchParams({ view });
  if (entityId) query.set("entityId", entityId);
  return request(`/scheduling/overview?${query.toString()}`);
}

export function getManualSchedulingContext(sectionId) {
  return request(`/scheduling/sections/${sectionId}/manual-context`);
}

export function saveManualAssignment(sectionId, data) {
  return request(`/scheduling/sections/${sectionId}/manual-assignment`, {
    method: "PUT",
